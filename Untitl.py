...
# Upload a zip file containing the garbage classification dataset
uploaded_file = st.file_uploader("Upload your dataset (zip file):", type=["zip"])

if uploaded_file is not None:
    with open("garbage_classification.zip", "wb") as f:
    ...
    # Specify the extracted directory as base_dir
    base_dir = "garbage_classification"
    st.write(f"Base directory set to: {base_dir}")

    # Example: List class names
    for class_name in os.listdir(base_dir):
        st.write(f"Class found: {class_name}")
